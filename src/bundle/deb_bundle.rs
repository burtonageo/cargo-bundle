...
pub fn bundle_project(settings: &Settings) -> Result<Vec<PathBuf>, Box<Error + Send + Sync>> {
    ...
    let bin_file_metadata = {
        let bin_file = try!(File::open(&settings.cargo_settings.binary_file));
        try!(bin_file.metadata())
    };
    ...
    try!(copy_file_to_dir(&settings.cargo_settings.binary_file,
                          data_dir.join("usr/bin")));
    try!(transfer_resource_files(settings, &data_dir));
    try!(generate_desktop_file(settings, &data_dir));
    ...
    try!(create_file_with_data(&control_dir.join("control"), &control_file_contents));
    try!(generate_md5sums(&control_dir, &data_dir));

    // Generate `debian-binary` file; see
    // http://www.tldp.org/HOWTO/Debian-Binary-Package-Building-HOWTO/x60.html#AEN66
    try!(create_file_with_data(package_dir.join("debian-binary"), "2.0\n"));
...
fn generate_desktop_file(settings: &Settings, data_dir: &Path) -> Result<(), Box<Error + Send + Sync>> {
    let bin_name = try!(settings.cargo_settings.binary_name());
    ...
    try!(create_file_with_data(desktop_file_path, &desktop_file_contents));
    Ok(())
}
...
fn generate_md5sums(control_dir: &Path, data_dir: &Path) -> io::Result<()> {
    ...
    let mut md5sums_file = try!(create_empty_file(&md5sums_path));
    for entry in WalkDir::new(data_dir) {
        ...
        try!(io::copy(&mut file, &mut hash));
        for byte in hash.compute().iter() {
            try!(write!(md5sums_file, "{:02x}", byte));
        }
        let rel_path = path.strip_prefix(data_dir).unwrap();
        let path_str = try!(rel_path.to_str().ok_or_else(|| {
            let msg = format!("Non-UTF-8 path: {:?}", rel_path);
            io::Error::new(io::ErrorKind::InvalidData, msg)
        }));
        try!(write!(md5sums_file, "  {}\n", path_str));
    }
    Ok(())
}
...
/// `data_dir`.
fn transfer_resource_files(settings: &Settings, data_dir: &Path) -> Result<(), Box<Error + Send + Sync>> {
    let bin_name = try!(settings.cargo_settings.binary_name());
    ...
    for res_path in &settings.resource_files {
        for entry in WalkDir::new(res_path) {
            ...
            let dest_dir = if src_path.is_absolute() {
                ...
                resource_dir.join(try!(src_path.parent().ok_or_else(|| {
                    let msg = format!("Not a file path: {:?}", src_path);
                    io::Error::new(io::ErrorKind::InvalidInput, msg)
                })))
            };
            try!(copy_file_to_dir(src_path, dest_dir));
        }
    }
    Ok(())
}
...
fn create_empty_file<P: AsRef<Path>>(path: P) -> io::Result<BufWriter<File>> {
    ...
    let dir_path = try!(file_path.parent().ok_or_else(|| {
        let msg = format!("Not a file path: {:?}", file_path);
        io::Error::new(io::ErrorKind::InvalidInput, msg)
    }));
    try!(create_dir_all(dir_path));
...
fn create_file_with_data<P: AsRef<Path>>(path: P, data: &str) -> io::Result<()> {
    let mut file = try!(create_empty_file(path));
...
fn copy_file_to_dir<P: AsRef<Path>, Q: AsRef<Path>>(file_path: P, dir_path: Q) -> io::Result<()> {
    ...
    let file_name = try!(file_path.file_name().ok_or_else(|| {
        let msg = format!("Not a file path: {:?}", file_path);
        io::Error::new(io::ErrorKind::InvalidInput, msg)
    }));
    try!(create_dir_all(dir_path));
    try!(fs::copy(file_path, dir_path.join(file_name)));
    Ok(())
}
