...
pub fn bundle_project(settings: &Settings) -> ::Result<Vec<PathBuf>> {
    ...
    let package_base_name = format!("{}_{}_{}", settings.binary_name(), settings.version_string(), arch);
    let package_name = format!("{}.deb", package_base_name);
    common::print_bundling(&package_name)?;
    let base_dir = settings.project_out_directory().join("bundle/deb");
    let package_dir = base_dir.join(&package_base_name);
    if package_dir.exists() {
        fs::remove_dir_all(&package_dir).chain_err(|| format!("Failed to remove old {}", package_base_name))?;
    }
    ...
    common::copy_file(settings.binary_path(), &binary_dest).chain_err(|| "Failed to copy binary file")?;
    ...
    generate_control_file(settings, arch, &control_dir, &data_dir).chain_err(|| "Failed to create control file")?;
    ...
    let control_tar_gz_path = tar_and_gzip_dir(control_dir).chain_err(|| "Failed to tar/gzip control directory")?;
    let data_tar_gz_path = tar_and_gzip_dir(data_dir).chain_err(|| "Failed to tar/gzip data directory")?;
    create_archive(
