...
pub fn bundle_project(settings: &Settings) -> ::Result<Vec<PathBuf>> {
    ...
    if bundle_dir.exists() {
        fs::remove_dir_all(&bundle_dir).chain_err(|| format!("Failed to remove old {}", app_bundle_name))?;
    }
    fs::create_dir_all(&bundle_dir).chain_err(|| format!("Failed to create bundle directory at {:?}", bundle_dir))?;
    ...
    let icon_filenames = generate_icon_files(&bundle_dir, settings).chain_err(|| "Failed to create app icons")?;
    generate_info_plist(&bundle_dir, settings, &icon_filenames).chain_err(|| "Failed to create Info.plist")?;
