...
mod tests {
    ...
    fn copy_dir_with_symlinks() {
        ...
        symlink_file(&PathBuf::from("sub/file.txt"), &tmp.path().join("orig/link")).unwrap();
        assert_eq!(
            std::fs::read(tmp.path().join("orig/link")).unwrap().as_slice(),
            b"Hello, world!\n"
        );
        ...
        assert_eq!(
            std::fs::read(tmp.path().join("parent/copy/link")).unwrap().as_slice(),
            b"Hello, world!\n"
        );
    }
